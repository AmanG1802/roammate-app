"""§29 Roles & RBAC (DB) — DB-backed role resolution helpers."""
...
from app.services.roles import get_trip_member, require_trip_member, require_trip_admin, require_vote_role
...
async def _setup(db, role, status="accepted"):
...
async def test_get_trip_member_returns_none_for_non_member(db_session):
    ...
    assert await get_trip_member(db_session, trip.id, u.id) is None
