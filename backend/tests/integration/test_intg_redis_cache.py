"""§26 Redis Cache & Breaker — Redis-backed cache roundtrips and circuit breaker."""
...
async def test_redis_breaker_opens_after_threshold(fake_redis):
    ...
    assert b.state == "open" and await b.allow() is False
...
async def test_redis_breaker_success_closes(fake_redis):
    ...
    assert await b.allow() is True and b.state == "closed"
